...
function handle_request(env)
    ...
    if vars["action"] == "start" then
        command = string.format("lua %s start -s %d -r %d -l %d -R %d -t assoc,batman,scan", script_location, vars["sleep_time"], vars["run_time"], vars["max_lines"], vars["reset_file"])
    elseif vars["action"] == "restart" then
        command = string.format("lua %s restart -s %d -r %d -l %d -R %d -t assoc,batman,scan", script_location, vars["sleep_time"], vars["run_time"], vars["max_lines"], vars["reset_file"])
